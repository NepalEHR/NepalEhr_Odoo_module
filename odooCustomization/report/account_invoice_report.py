...
class AccountInvoiceReport(models.Model):
    _name = "custom.account.invoice.report"
    _description = "Customized  Invoices Statistics"
    _auto = False
    _rec_name = 'date'
    ...
    discount = fields.Float(string='Total Discount', readonly=True)
    total_amount = fields.Float(string='Total Amount After Discount', readonly=True)
    ...
    def _select(self):
        select_str = """
            SELECT sub.id, sub.date, sub.product_id, sub.partner_id, sub.country_id, sub.account_analytic_id,
                ...
                sub.fiscal_position_id, sub.user_id,sub.team_id, sub.company_id, sub.nbr, sub.type, sub.state,
                sub.weight, sub.volume,
                sub.categ_id, sub.date_due, sub.account_id, sub.account_line_id, sub.partner_bank_id,
                sub.product_qty, sub.price_total as price_total,sub.discount,sub.total_amount, sub.price_average as price_average,
    ...
    def _sub_select(self):
        select_str = """
                SELECT ail.id AS id,
                    ...
                    ((ai.discount_percentage/100) * (ail.price_subtotal_signed * invoice_type.sign)) as discount,
                    SUM((ail.price_subtotal_signed * invoice_type.sign)  - ((ai.discount_percentage/100) * (ail.price_subtotal_signed * invoice_type.sign))) AS total_amount,
    ...
    def _from(self):
        from_str = """
                ...
                LEFT JOIN crm_team crt on crt.id =ru.sale_team_id
                JOIN (
    ...
    def _group_by(self):
        group_by_str = """
                GROUP BY ail.id, ail.product_id, ail.account_analytic_id, ai.date_invoice, ai.id,
                    ...
                    ai.fiscal_position_id, ai.user_id,crt.id, ai.company_id, ai.type, invoice_type.sign, ai.state, pt.categ_id,
